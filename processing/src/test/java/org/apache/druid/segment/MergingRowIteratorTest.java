...
public class MergingRowIteratorTest {
	...
	private static void populateSequences(List<List<Long>> possibleSequences, Deque<Long> currentSequence, int current,
			int max, int maxDepth) {
	...
	private static void testMerge(String message, int markIteration, List<Long>... timestampSequences) {
		...
		while (mergedTimestamps.hasNext()) {
			...
			if (marked) {
				Assert.assertEquals(message, markedTimestamp != currentTimestamp,
	...
	private static class TestRowIterator implements TransformableRowIterator {
		...
		private TestRowIterator(Iterable<Long> timestamps) {
			...
			this.rowPointer = new RowPointer(currentTimestamp, ColumnValueSelector.EMPTY_ARRAY, Collections.emptyList(),
					ColumnValueSelector.EMPTY_ARRAY, Collections.emptyList(), rowNumCounter);
			this.markedRowPointer = new TimeAndDimsPointer(markedTimestamp, ColumnValueSelector.EMPTY_ARRAY,
					Collections.emptyList(), ColumnValueSelector.EMPTY_ARRAY, Collections.emptyList());
		}
