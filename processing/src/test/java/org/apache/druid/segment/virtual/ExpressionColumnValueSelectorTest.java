...
public class ExpressionColumnValueSelectorTest {
	...
	public void testSupplierFromDimensionSelector() {
		...
		final Supplier<Object> supplier = ExpressionSelectors
				.supplierFromDimensionSelector(dimensionSelectorFromSupplier(settableSupplier), false);
	...
	public void testSupplierFromObjectSelectorObject() {
		...
		final Supplier<Object> supplier = ExpressionSelectors
				.supplierFromObjectSelector(objectSelectorFromSupplier(settableSupplier, Object.class));
	...
	public void testSupplierFromObjectSelectorNumber() {
		...
		final Supplier<Object> supplier = ExpressionSelectors
				.supplierFromObjectSelector(objectSelectorFromSupplier(settableSupplier, Number.class));
	...
	public void testSupplierFromObjectSelectorString() {
		final SettableSupplier<String> settableSupplier = new SettableSupplier<>();
		final Supplier<Object> supplier = ExpressionSelectors
				.supplierFromObjectSelector(objectSelectorFromSupplier(settableSupplier, String.class));
	...
	public void testSupplierFromObjectSelectorList() {
		...
		final Supplier<Object> supplier = ExpressionSelectors
				.supplierFromObjectSelector(objectSelectorFromSupplier(settableSupplier, List.class));
	...
	public void testCoerceListToArray() {
		...
		Assert.assertArrayEquals(new Double[] { 1.0, 2.0, 3.0 },
				(Double[]) ExpressionSelectors.coerceListToArray(floatList));

		List<Double> doubleList = ImmutableList.of(1.0, 2.0, 3.0);
		Assert.assertArrayEquals(new Double[] { 1.0, 2.0, 3.0 },
				(Double[]) ExpressionSelectors.coerceListToArray(doubleList));

		List<String> stringList = ImmutableList.of("a", "b", "c");
		Assert.assertArrayEquals(new String[] { "a", "b", "c" },
				(String[]) ExpressionSelectors.coerceListToArray(stringList));

		List<String> withNulls = new ArrayList<>();
		withNulls.add("a");
		withNulls.add(null);
		withNulls.add("c");
		Assert.assertArrayEquals(new String[] { "a", null, "c" },
				(String[]) ExpressionSelectors.coerceListToArray(withNulls));
		...
		Assert.assertArrayEquals(new Long[] { 1L, null, 3L },
				(Long[]) ExpressionSelectors.coerceListToArray(withNumberNulls));

		List<Object> withStringMix = ImmutableList.of(1L, "b", 3L);
		Assert.assertArrayEquals(new String[] { "1", "b", "3" },
	...
	public void testCoerceExprToValue() {
		...
		withNulls.add(null);
		withNulls.add("c");
		Assert.assertEquals(withNulls, ExpressionSelectors
				.coerceEvalToSelectorObject(ExprEval.ofStringArray(new String[] { "a", null, "c" })));
	}
	...
	private static <T> ColumnValueSelector<T> objectSelectorFromSupplier(final Supplier<T> supplier,
			final Class<T> clazz) {
