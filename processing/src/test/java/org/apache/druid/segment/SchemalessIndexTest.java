...
public class SchemalessIndexTest {
			...
			for (final Map<String, Object> event : EVENTS) {
				...
				if (theIndex == null) {
					theIndex = new IncrementalIndex.Builder()
							.setIndexSchema(new IncrementalIndexSchema.Builder().withMinTimestamp(timestamp)
									.withQueryGranularity(Granularities.MINUTE).withMetrics(METRIC_AGGS).build())
	...
	public QueryableIndex getMergedIncrementalIndex() {
		synchronized (log) {
			...
			try {
				...
				mergedIndex = indexIO.loadIndex(indexMerger.mergeQueryableIndex(
						Arrays.asList(indexIO.loadIndex(topFile), indexIO.loadIndex(bottomFile)), true, METRIC_AGGS,
						mergedFile, INDEX_SPEC, null));
		...
		synchronized (log) {
			...
			try {
				...
				QueryableIndex index = indexIO.loadIndex(indexMerger.mergeQueryableIndex(
						Arrays.asList(ROW_PERSISTED_INDEXES.get(index1), ROW_PERSISTED_INDEXES.get(index2)), true,
						METRIC_AGGS, mergedFile, INDEX_SPEC, null));
	...
	public QueryableIndex getMergedIncrementalIndex(int[] indexes) {
		synchronized (log) {
			...
			try {
				...
				return indexIO.loadIndex(indexMerger.mergeQueryableIndex(indexesToMerge, true, METRIC_AGGS, mergedFile,
						INDEX_SPEC, null));
			} catch (IOException e) {
				throw new RuntimeException(e);
			}
	...
	public QueryableIndex getAppendedIncrementalIndex(Iterable<Pair<String, AggregatorFactory[]>> files,
	...
	public QueryableIndex getMergedIncrementalIndexDiffMetrics() {
		return getMergedIncrementalIndex(
				Arrays.asList(new Pair<String, AggregatorFactory[]>("druid.sample.json.top", METRIC_AGGS_NO_UNIQ),
		...
		synchronized (log) {
			try {
				...
				for (final Map<String, Object> event : EVENTS) {
					...
					final long timestamp = new DateTime(event.get(TIMESTAMP), ISOChronology.getInstanceUTC())
							.getMillis();
					final List<String> dims = new ArrayList<>();
	...
	public static IncrementalIndex makeIncrementalIndex(final String resourceFilename, AggregatorFactory[] aggs) {
		...
		final IncrementalIndex retVal = new IncrementalIndex.Builder()
				.setIndexSchema(new IncrementalIndexSchema.Builder()
						...
						.withQueryGranularity(Granularities.MINUTE).withMetrics(aggs).build())
				.setMaxRowCount(1000).buildOnheap();
		...
		try {
			...
			for (Object obj : events) {
				...
				retVal.add(new MapBasedInputRow(
						new DateTime(event.get(TIMESTAMP), ISOChronology.getInstanceUTC()).getMillis(), dims, event));
			}
		} catch (IOException e) {
			index = null;
			throw new RuntimeException(e);
	...
	private List<File> makeFilesToMap(File tmpFile, Iterable<Pair<String, AggregatorFactory[]>> files)
			throws IOException {
	...
	private QueryableIndex makeAppendedMMappedIndex(Iterable<Pair<String, AggregatorFactory[]>> files,
		...
		try {
			...
			for (int i = 0; i < intervals.size(); i++) {
				timeline.add(intervals.get(i), i,
						noneShardSpec.createChunk(new OvershadowableFile(i, filesToMap.get(i))));
			}

			final List<IndexableAdapter> adapters = Lists.newArrayList(Iterables.concat(
					// TimelineObjectHolder is actually an iterable of iterable
					// of indexable adapters
					Iterables.transform(timeline.lookup(Intervals.of("1000-01-01/3000-01-01")),
							new Function<TimelineObjectHolder<Integer, OvershadowableFile>, Iterable<IndexableAdapter>>() {
								...
								public Iterable<IndexableAdapter> apply(
										final TimelineObjectHolder<Integer, OvershadowableFile> timelineObjectHolder) {
									return Iterables.transform(timelineObjectHolder.getObject(),

											// Each chunk can be used to build
											// the actual IndexableAdapter
											new Function<PartitionChunk<OvershadowableFile>, IndexableAdapter>() {
												@Override
												public IndexableAdapter apply(
														PartitionChunk<OvershadowableFile> chunk) {
													try {
														return new RowFilteringIndexAdapter(
																new QueryableIndexIndexableAdapter(
																		indexIO.loadIndex(chunk.getObject().file)),
																rowPointer -> timelineObjectHolder.getInterval()
																		.contains(rowPointer.getTimestamp()));
													} catch (IOException e) {
														throw new RuntimeException(e);
													}
												}
	...
	private QueryableIndex makeMergedMMappedIndex(Iterable<Pair<String, AggregatorFactory[]>> files) {
		try {
			...
			return indexIO.loadIndex(indexMerger.mergeQueryableIndex(
					...
					})), true, METRIC_AGGS, mergedFile, INDEX_SPEC, null));
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
	}
