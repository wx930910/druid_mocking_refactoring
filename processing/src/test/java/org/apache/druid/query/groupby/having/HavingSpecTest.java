...
public class HavingSpecTest {
	...
	public void testEqualHavingSpec() {
		...
		Assert.assertFalse(spec.eval(getTestRow(100.56d))); // False since
															// 100.56d !=
															// (double) 100.56f
		...
		Assert.assertFalse(spec.eval(getTestRow(90.53d)));
		Assert.assertFalse(spec.eval(getTestRow(100.56f))); // False since
															// 100.56d !=
															// (double) 100.56f
		Assert.assertFalse(spec.eval(getTestRow(90.53f)));
		Assert.assertFalse(spec.eval(getTestRow(Long.MAX_VALUE)));
	...
	public void testAndHavingSpecShouldSupportShortcutEvaluation() {
		...
		// Mock first instance
		HavingSpec spec1 = Mockito.mock(HavingSpec.class);
		boolean value1 = true;
		Mockito.when(spec1.eval(Mockito.any())).thenAnswer(invo -> {
			counter.incrementAndGet();
			return value1;
		});
		Mockito.when(spec1.getCacheKey()).thenReturn(
				new CacheKeyBuilder(HavingSpecUtil.CACHE_TYPE_ID_COUNTING).appendByte((byte) (value1 ? 1 : 0))
						.appendByteArray(StringUtils.toUtf8(String.valueOf(counter))).build());
		// Mock second instance
		HavingSpec spec2 = Mockito.mock(HavingSpec.class);
		boolean value2 = false;
		Mockito.when(spec2.eval(Mockito.any())).thenAnswer(invo -> {
			counter.incrementAndGet();
			return value2;
		});
		Mockito.when(spec2.getCacheKey()).thenReturn(
				new CacheKeyBuilder(HavingSpecUtil.CACHE_TYPE_ID_COUNTING).appendByte((byte) (value2 ? 1 : 0))
						.appendByteArray(StringUtils.toUtf8(String.valueOf(counter))).build());
		// Mock third instance
		HavingSpec spec3 = Mockito.mock(HavingSpec.class);
		boolean value3 = true;
		Mockito.when(spec3.eval(Mockito.any())).thenAnswer(invo -> {
			counter.incrementAndGet();
			return value3;
		});
		Mockito.when(spec3.getCacheKey()).thenReturn(
				new CacheKeyBuilder(HavingSpecUtil.CACHE_TYPE_ID_COUNTING).appendByte((byte) (value3 ? 1 : 0))
						.appendByteArray(StringUtils.toUtf8(String.valueOf(counter))).build());
		// Mock fourth instance
		HavingSpec spec4 = Mockito.mock(HavingSpec.class);
		boolean value4 = false;
		Mockito.when(spec4.eval(Mockito.any())).thenAnswer(invo -> {
			counter.incrementAndGet();
			return value4;
		});
		Mockito.when(spec4.getCacheKey()).thenReturn(
				new CacheKeyBuilder(HavingSpecUtil.CACHE_TYPE_ID_COUNTING).appendByte((byte) (value4 ? 1 : 0))
						.appendByteArray(StringUtils.toUtf8(String.valueOf(counter))).build());

		// Modify caller
		AndHavingSpec spec = new AndHavingSpec(ImmutableList.of(spec1, spec2, spec3, spec4));
	...
	public void testOrHavingSpecSupportsShortcutEvaluation() {
		...
		OrHavingSpec spec = new OrHavingSpec(
				ImmutableList.of(new CountingHavingSpec(counter, true), new CountingHavingSpec(counter, true),
						new CountingHavingSpec(counter, true), new CountingHavingSpec(counter, false)));
	...
	public void testOrHavingSpec() {
		AtomicInteger counter = new AtomicInteger(0);
		OrHavingSpec spec = new OrHavingSpec(
				ImmutableList.of(new CountingHavingSpec(counter, false), new CountingHavingSpec(counter, false),
						new CountingHavingSpec(counter, false), new CountingHavingSpec(counter, false)));
		...
		counter.set(0);
		spec = new OrHavingSpec(
				ImmutableList.of(new CountingHavingSpec(counter, false), new CountingHavingSpec(counter, false),
						new CountingHavingSpec(counter, false), new CountingHavingSpec(counter, true)));
