...
public class MetricManipulatorFnsTest {
	...
	public static Iterable<Object[]> constructorFeeder() {
		...
		LongColumnSelector selector = Mockito.mock(LongColumnSelector.class);
		Mockito.when(selector.getLong()).thenReturn(longVal);

		LongMinAggregator longMinAggregator = new LongMinAggregator(selector);
		LongMinAggregatorFactory longMinAggregatorFactory = new LongMinAggregatorFactory(NAME, FIELD);
		constructorArrays.add(new Object[] { longMinAggregatorFactory, longMinAggregator, longMinAggregator,
				longMinAggregator, longVal, longVal });
		...
		constructorArrays.add(new Object[] { hyperUniquesAggregatorFactory, collector, collector,
				collector.estimateCardinality(), collector.toByteArray(), collector });
		...
		constructorArrays.add(new Object[] { longSumAggregatorFactory, longSumAggregator, longSumAggregator,
				longSumAggregator, longVal, longVal });

		for (Object[] argList : constructorArrays) {
			Assert.assertEquals(StringUtils.format("Arglist %s is too short. Expected 6 found %d",
					Arrays.toString(argList), argList.length), 6, argList.length);
		}
	...
	public MetricManipulatorFnsTest(AggregatorFactory aggregatorFactory, Object agg, Object identity, Object finalize,
			Object serialForm, Object deserForm) {
