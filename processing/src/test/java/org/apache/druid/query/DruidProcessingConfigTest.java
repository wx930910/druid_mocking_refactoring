...
public class DruidProcessingConfigTest {
	...
	private static Injector makeInjector(int numProcessors, long directMemorySize, long heapSize, Properties props,
		...
		return Guice.createInjector(binder -> {
			binder.bind(RuntimeInfo.class).toInstance(mockRuntimeInfo(numProcessors, directMemorySize, heapSize));
	...
	private static RuntimeInfo mockRuntimeInfo(final int availableProcessors, final long directSize,
			final long maxHeapSize) {
		RuntimeInfo res = Mockito.mock(RuntimeInfo.class);
		Mockito.when(res.getAvailableProcessors()).thenAnswer(invo -> {
			return availableProcessors;
		});
		Mockito.when(res.getMaxHeapSizeBytes()).thenAnswer(invo -> {
			return maxHeapSize;
		});
		Mockito.when(res.getDirectMemorySizeBytes()).thenAnswer(invo -> {
			return directSize;
		});
		return res;
	}
