...
public class KafkaSupervisorTest extends EasyMockSupport {
	...
	/**
	 * Test generating the starting offsets from the partition high water marks
	 * in Kafka.
	 */
	@Test
	...
	public void testBadMetadataOffsets() throws Exception {
		...
		EasyMock.expect(taskMaster.getTaskRunner()).andReturn(Optional.absent()).anyTimes();
		EasyMock.expect(taskStorage.getActiveTasksByDatasource(DATASOURCE)).andReturn(ImmutableList.of()).anyTimes();
		// for simplicity in testing the offset availability check, we use
		// negative stored offsets in metadata here,
		// because the stream's earliest offset is 0, although that would not
	...
	public void testRequeueAdoptedTaskWhenFailed() throws Exception {
		...
		// check that failed tasks are recreated with the same
		// minimumMessageTime as the task it replaced, even if that
	...
	public void testQueueNextTasksOnSuccess() throws Exception {
		...
		// test that a task succeeding causes a new task to be re-queued with
		// the next offset range and causes any replica
		...
		// make sure we killed the right task (sequenceName for replicas are the
		// same)
	...
	public void testDiscoverExistingPublishingTask() throws Exception {
		...
		// check that the new task was created with starting offsets matching
		// where the publishing task finished
	...
	public void testStopGracefully() throws Exception {
		...
		// getCheckpoints will not be called for id1 as it is in publishing
		// state
		TreeMap<Integer, Map<Integer, Long>> checkpoints = new TreeMap<>();
	...
	public void testResetDataSourceMetadata() throws Exception {
		...
		try {
			...
			// Expected as there will be an attempt to EasyMock.reset
			// partitionGroups offsets to NOT_SET
			// however there would be no entries in the map as we have not put
			// nay data in kafka
	...
	public void testGetOffsetFromStorageForPartitionWithResetOffsetAutomatically() throws Exception {
		...
		EasyMock.expect(indexerMetadataStorageCoordinator.retrieveDataSourceMetadata(DATASOURCE))
				...
				.times(4);
		// getOffsetFromStorageForPartition() throws an exception when the
		// offsets are automatically reset.
		...
		EasyMock.expect(indexerMetadataStorageCoordinator.resetDataSourceMetadata(DATASOURCE,
				new KafkaDataSourceMetadata(
						// Only one partition is reset in a single supervisor
						// run.
	...
	public void testSuspendedRunningTasks() throws Exception {
		// graceful shutdown is expected to be called on running tasks since
		// state is suspended
	...
	public void testFailedInitializationAndRecovery() throws Exception {
		// Block the supervisor initialization with a bad hostname config, make
		// sure this doesn't block the lifecycle
		...
		// The test supervisor has a P1D period, so we need to manually trigger
		// the initialization retry.
	...
	public void testDoNotKillCompatibleTasks() throws Exception {
		// This supervisor always returns true for isTaskCurrent -> it should
		// not kill its tasks
		int numReplicas = 2;
	...
	public void testIsTaskCurrent() {
		...
		KafkaSupervisorTuningConfig modifiedTuningConfig = new KafkaSupervisorTuningConfig(42, // This
																								// is
																								// different
	...
	private TestableKafkaSupervisor getTestableSupervisor(int replicas, int taskCount, boolean useEarliestOffset,
		...
		return new TestableKafkaSupervisor(taskStorage, taskMaster, indexerMetadataStorageCoordinator,
				...
				new KafkaSupervisorSpec(null, dataSchema, tuningConfig, kafkaSupervisorIOConfig, null, suspended,
						...
						new NoopServiceEmitter(), new DruidMonitorSchedulerConfig(), rowIngestionMetersFactory,
	...
	// private TaskRunnerWorkItem mockTaskRunnerWorkItem(Task task,
	// ListenableFuture<TaskStatus> result,
	// TaskLocation location) {
	// // TaskRunnerWorkItem res = Mockito.
	// }
