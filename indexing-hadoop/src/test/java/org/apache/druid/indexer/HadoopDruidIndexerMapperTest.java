...
public class HadoopDruidIndexerMapperTest {
	...
	public void testHadoopyStringParserWithTransformSpec() throws Exception {
		final HadoopDruidIndexerConfig config = new HadoopDruidIndexerConfig(
				new HadoopIngestionSpec(
						DATA_SCHEMA.withTransformSpec(new TransformSpec(new SelectorDimFilter("dim1", "foo", null),
								ImmutableList.of(
		...
		assertRowListEquals(ImmutableList.of(
				ImmutableMap.of("t", "2000-01-01T00:00:00.000Z", "dim1", "foo", "dim1t", "foofoo", "dim2", "x", "m1",
						1.0),
				ImmutableMap.of("t", "2000-01-01T00:00:00.000Z", "dim1", "foo", "dim1t", "foofoo", "dim2", "z", "m1",
						1.0)),
				mapper.getRows());
	}
	...
	private static Map<String, Object> rowToMap(final InputRow row) {
		...
		// other, non-dimension fields are not self describing so much be
