...
public class PendingTaskBasedProvisioningStrategyTest {
	...
	public void testSuccessfulMinWorkersProvision() {
		...
		EasyMock.expect(runner.getWorkers())
				.andReturn(Collections.singletonList(new TestZkWorker(testTask).toImmutable()));
		EasyMock.expect(runner.getConfig()).andReturn(new RemoteTaskRunnerConfig());
		EasyMock.expect(autoScaler.provision()).andReturn(new AutoScalingData(Collections.singletonList("aNode")))
				.times(2);
		EasyMock.replay(runner, autoScaler);
	...
	public void testSuccessfulMinWorkersProvisionWithOldVersionNodeRunning() {
		...
		EasyMock.expect(runner.getWorkers()).andReturn(Arrays.asList(new TestZkWorker(testTask).toImmutable(),
				new TestZkWorker(testTask, "http", "h1", "n1", INVALID_VERSION).toImmutable() // Invalid
																								// version
																								// node
		));
		EasyMock.expect(runner.getConfig()).andReturn(new RemoteTaskRunnerConfig());
	...
	public void testProvisionAlert() throws Exception {
		...
		EasyMock.expect(runner.getPendingTaskPayloads()).andReturn(Collections.singletonList(NoopTask.create()))
				.times(2);
		EasyMock.expect(runner.getWorkers())
				.andReturn(Arrays.asList(new TestZkWorker(testTask, "http", "hi", "lo", MIN_VERSION, 1).toImmutable(),
						new TestZkWorker(testTask, "http", "h1", "n1", INVALID_VERSION).toImmutable(), // Invalid
																										// version
																										// node
						new TestZkWorker(testTask, "http", "h2", "n1", INVALID_VERSION).toImmutable() // Invalid
																										// version
																										// node
				)).times(2);
		EasyMock.expect(runner.getConfig()).andReturn(new RemoteTaskRunnerConfig());
	...
	public void testDoSuccessfulTerminate() {
		...
		EasyMock.expect(runner.getPendingTasks())
				...
				.times(2);
		EasyMock.expect(runner.getWorkers()).andReturn(Collections.singletonList(mockWorker(testTask).toImmutable()))
				.times(2);
	...
	public void testSomethingTerminating() {
		...
		RemoteTaskRunner runner = EasyMock.createMock(RemoteTaskRunner.class);
		EasyMock.expect(runner.getWorkers())
				.andReturn(Collections.singletonList(new TestZkWorker(testTask).toImmutable())).times(2);
	...
	public void testNoActionNeeded() {
		...
		EasyMock.expect(runner.getPendingTaskPayloads()).andReturn(Collections.singletonList((Task) NoopTask.create()))
				.times(1);
		EasyMock.expect(runner.getWorkers()).andReturn(Arrays.asList(new TestZkWorker(NoopTask.create()).toImmutable(),
				new TestZkWorker(NoopTask.create()).toImmutable())).times(2);
		EasyMock.expect(runner.getConfig()).andReturn(new RemoteTaskRunnerConfig());
	...
	public void testMinCountIncrease() {
		...
		EasyMock.expect(runner.getWorkers()).andReturn(Collections
				.singletonList(new TestZkWorker(NoopTask.create(), "http", "h1", "i1", MIN_VERSION).toImmutable()))
				.times(3);
	...
	public void testNullWorkerConfig() {
		...
		EasyMock.expect(runner.getPendingTaskPayloads()).andReturn(Collections.singletonList(NoopTask.create()))
				.times(1);
		EasyMock.expect(runner.getWorkers()).andReturn(Collections.singletonList(new TestZkWorker(null).toImmutable()))
				.times(2);
		EasyMock.replay(runner);
	...
	private ZkWorker mockWorker(Task testTask) {
		ZkWorker res = Mockito.spy(new ZkWorker(new Worker("http", "host", "ip", 3, "0", WorkerConfig.DEFAULT_CATEGORY),
				null, new DefaultObjectMapper()));
		Mockito.doAnswer(invo -> {
			if (testTask == null) {
				return new HashMap<>();
			}
			return ImmutableMap.of(testTask.getId(),
					TaskAnnouncement.create(testTask, TaskStatus.running(testTask.getId()), TaskLocation.unknown()));
		}).when(res).getRunningTasks();
		return res;
	}
	...
	private static class TestZkWorker extends ZkWorker {
		...
		public TestZkWorker(Task testTask, String scheme, String host, String ip, String version, int capacity) {
			super(new Worker(scheme, host, ip, capacity, version, WorkerConfig.DEFAULT_CATEGORY), null,
