...
public class PerfectRollupWorkerTaskTest {
	...
	private static class PerfectRollupWorkerTaskBuilder {
		private static final PartitionsSpec PARTITIONS_SPEC = new HashedPartitionsSpec(null, 1, null, null, null);
		...
		PerfectRollupWorkerTask build() {
			// PerfectRollupWorkerTask res =
			// Mockito.mock(PerfectRollupWorkerTask.class,
			// Mockito.withSettings().defaultAnswer(Mockito.CALLS_REAL_METHODS).useConstructor("id",
			// "group-id",
			// null, createDataSchema(granularitySpecInputIntervals),
			// createTuningConfig(forceGuaranteedRollup, partitionsSpec),
			// null));
			// try {
			// Mockito.when(res.runTask(Mockito.any())).thenThrow(new
			// UnsupportedOperationException());
			// Mockito.when(res.getType()).thenThrow(new
			// UnsupportedOperationException());
			// Mockito.when(res.isReady(Mockito.any())).thenThrow(new
			// UnsupportedOperationException());
			// } catch (Exception e) {
			// e.printStackTrace();
			// }
			return new TestPerfectRollupWorkerTask("id", "group-id", null,
					createDataSchema(granularitySpecInputIntervals),
					createTuningConfig(forceGuaranteedRollup, partitionsSpec), null);
		}
		...
		private static ParallelIndexTuningConfig createTuningConfig(boolean forceGuaranteedRollup,
	...
	private static class TestPerfectRollupWorkerTask extends PerfectRollupWorkerTask {
		TestPerfectRollupWorkerTask(String id, @Nullable String groupId, @Nullable TaskResource taskResource,
				DataSchema dataSchema, ParallelIndexTuningConfig tuningConfig, @Nullable Map<String, Object> context) {
