...
public class OverlordTest {
	...
	public void setUp() throws Exception {
		...
		taskMaster = new TaskMaster(new TaskLockConfig(),
				...
				taskActionClientFactory, druidNode, new TaskRunnerFactory<MockTaskRunner>() {
					@Override
				...
				EasyMock.createNiceMock(OverlordHelperManager.class), mockDruidSelector());
	...
	public void testOverlordRun() throws Exception {
		...
		while (!taskMaster.isLeader()) {
			// I believe the control will never reach here and thread will never
			// sleep but just to be on safe side
			Thread.sleep(10);
		}
	...
	/*
	 * Wait until the task with given taskId has the given Task Status These
	 * method will not timeout until the condition is met so calling method
	 * should ensure timeout This method also assumes that the task with given
	 * taskId is present
	 */
	...
	public static class MockTaskRunner implements TaskRunner {
			...
			ListenableFuture<TaskStatus> future = MoreExecutors
					...
					.submit(new Callable<TaskStatus>() {
						...
						public TaskStatus call() throws Exception {
							// adding of task to list of runningTasks should be
							// done before count down as
							// getRunningTasks may not include the task for
							// which latch has been counted down
							// Count down to let know that task is actually
							// running
							// this is equivalent of getting process holder to
							// run task in ForkingTaskRunner
		...
		public synchronized Collection<? extends TaskRunnerWorkItem> getRunningTasks() {
			return Lists.transform(runningTasks, new Function<String, TaskRunnerWorkItem>() {
				@Nullable
				@Override
	...
	private DruidLeaderSelector mockDruidSelector() {
		org.apache.druid.discovery.DruidLeaderSelector.Listener[] listener = new org.apache.druid.discovery.DruidLeaderSelector.Listener[1];
		String[] leader = new String[1];
		DruidLeaderSelector res = Mockito.mock(DruidLeaderSelector.class);
		Mockito.when(res.getCurrentLeader()).thenAnswer(invo -> {
			return leader[0];
		});
		Mockito.when(res.isLeader()).thenAnswer(invo -> {
			return leader != null;
		});
		Mockito.when(res.localTerm()).thenReturn(0);
		Mockito.doAnswer(invo -> {
			org.apache.druid.discovery.DruidLeaderSelector.Listener l = invo.getArgument(0);
			listener[0] = l;
			leader[0] = "what:1234";
			listener[0].becomeLeader();
			return null;
		}).when(res).registerListener(Mockito.any());
		Mockito.doAnswer(invo -> {
			leader[0] = null;
			listener[0].stopBeingLeader();
			return null;
		}).when(res).unregisterListener();
		return res;
	}
