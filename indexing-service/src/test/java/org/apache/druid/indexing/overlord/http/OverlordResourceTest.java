...
public class OverlordResourceTest {
	...
	public void testSecuredGetWaitingTask() {
		...
		EasyMock.<Collection<? extends TaskRunnerWorkItem>> expect(taskRunner.getKnownTasks()).andReturn(
				ImmutableList.of(new MockTaskRunnerWorkItem("id_1", null), new MockTaskRunnerWorkItem("id_4", null)));
	...
	public void testSecuredGetCompleteTasks() {
		...
		EasyMock.<Collection<? extends TaskRunnerWorkItem>> expect(taskRunner.getRunningTasks())
				.andReturn(ImmutableList.of(new MockTaskRunnerWorkItem(tasksIds.get(0), null),
	...
	public void testGetTasks() {
		...
		EasyMock.<Collection<? extends TaskRunnerWorkItem>> expect(taskRunner.getKnownTasks()).andReturn(
				ImmutableList.of(mockTaskRunnerWorkItem("id_1", null), new MockTaskRunnerWorkItem("id_4", null)))
				.atLeastOnce();

		EasyMock.<Collection<? extends TaskRunnerWorkItem>> expect(taskRunner.getPendingTasks())
				.andReturn(ImmutableList.of(new MockTaskRunnerWorkItem("id_4", null)));

		EasyMock.<Collection<? extends TaskRunnerWorkItem>> expect(taskRunner.getRunningTasks())
				.andReturn(ImmutableList.of(new MockTaskRunnerWorkItem("id_1", null)));
	...
	private TaskRunnerWorkItem mockTaskRunnerWorkItem(String taskId, ListenableFuture<TaskStatus> result) {
		TaskRunnerWorkItem res = Mockito.mock(TaskRunnerWorkItem.class,
				Mockito.withSettings().defaultAnswer(Mockito.CALLS_REAL_METHODS).useConstructor(taskId, result));
		Mockito.when(res.getLocation()).thenAnswer(invo -> {
			return TaskLocation.unknown();
		});
		Mockito.when(res.getTaskType()).thenReturn("test");
		Mockito.when(res.getDataSource()).thenReturn("ds_test");
		return res;
	}
	...
	private static class MockTaskRunnerWorkItem extends TaskRunnerWorkItem {
		public MockTaskRunnerWorkItem(String taskId, ListenableFuture<TaskStatus> result) {
			super(taskId, result);
		}
