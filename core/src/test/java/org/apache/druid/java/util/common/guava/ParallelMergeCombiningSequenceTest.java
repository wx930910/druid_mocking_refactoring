...
public class ParallelMergeCombiningSequenceTest {
	...
	public static final Ordering<Pair> INT_PAIR_ORDERING = Ordering.natural().onResultOf(p -> (Integer) p.lhs);
	public static final BinaryOperator<Pair> INT_PAIR_MERGE_FN = (lhs, rhs) -> {
		if (lhs == null) {
			return rhs;
		}
		...
		return Mockito.spy(new Pair(lhs.lhs, (Integer) lhs.rhs + (Integer) rhs.rhs));
	...
	@Before
	public void setup() {
		pool = new ForkJoinPool(TEST_POOL_SIZE, ForkJoinPool.defaultForkJoinWorkerThreadFactory,
				(t, e) -> LOG.error(e, "Unhandled exception in thread [%s]", t), true);
	}
	...
	public void testOrderedResultBatchFromSequence() throws IOException {
		Sequence<Pair> rawSequence = nonBlockingSequence(5000);
		ParallelMergeCombiningSequence.YielderBatchedResultsCursor<Pair> cursor = new ParallelMergeCombiningSequence.YielderBatchedResultsCursor<>(
				new ParallelMergeCombiningSequence.SequenceBatcher<>(rawSequence, 128), INT_PAIR_ORDERING);
		cursor.initialize();
		Yielder<Pair> rawYielder = Yielders.each(rawSequence);
	...
	public void testOrderedResultBatchFromSequenceBackToYielderOnSequence() throws IOException {
		...
		Sequence<Pair> rawSequence = nonBlockingSequence(sequenceSize);
		ParallelMergeCombiningSequence.YielderBatchedResultsCursor<Pair> cursor = new ParallelMergeCombiningSequence.YielderBatchedResultsCursor<>(
				new ParallelMergeCombiningSequence.SequenceBatcher<>(rawSequence, 128), INT_PAIR_ORDERING);
		...
		ArrayBlockingQueue<ParallelMergeCombiningSequence.ResultBatch<Pair>> outputQueue = new ArrayBlockingQueue<>(
				(int) Math.ceil(((double) sequenceSize / batchSize) + 2));

		Pair prev = null;
		ParallelMergeCombiningSequence.ResultBatch<Pair> currentBatch = new ParallelMergeCombiningSequence.ResultBatch<>(
				batchSize);
		int batchCounter = 0;
		while (!rawYielder.isDone() && !cursor.isDone()) {
			Assert.assertEquals(rawYielder.get(), cursor.get());
		...
		Sequence<Pair> queueAsSequence = ParallelMergeCombiningSequence.makeOutputSequenceForQueue(outputQueue, true,
		...
		Yielder<Pair> queueYielder = Yielders.each(queueAsSequence);
	...
	public void testOrderedResultBatchFromSequenceToBlockingQueueCursor() throws IOException {
		...
		ParallelMergeCombiningSequence.BlockingQueueuBatchedResultsCursor<Pair> queueCursor = new ParallelMergeCombiningSequence.BlockingQueueuBatchedResultsCursor<>(
				outputQueue, INT_PAIR_ORDERING, false, -1L);
	...
	public void testNone() throws Exception {
		List<Sequence<Pair>> input = new ArrayList<>();
		assertResult(input);
	}
	...
	public void testAllInSingleBatch() throws Exception {
		...
		assertResult(input, 10, 20, reportMetrics -> {
			...
			// deltas because it depends how much result combining is happening,
			// which is random
	...
	public void testMultiBatchMultiYield() throws Exception {
		...
		// above min sequence count threshold, so will merge in parallel (if
		// enough cores)
		input.add(nonBlockingSequence(15));
	...
	public void testExceptionOnInputSequenceRead() throws Exception {
		...
		expectedException.expectMessage("exploded");
	...
	public void testTimeoutExceptionDueToStalledInput() throws Exception {
		...
		assertException(input, ParallelMergeCombiningSequence.DEFAULT_TASK_SMALL_BATCH_NUM_ROWS,
				ParallelMergeCombiningSequence.DEFAULT_TASK_INITIAL_YIELD_NUM_ROWS, 1000L, 0);
	}
	...
	public void testGracefulCloseOfYielderCancelsPool() throws Exception {
		...
		assertResultWithEarlyClose(input, 128, 1024, 256, reportMetrics -> {
			Assert.assertEquals(2, reportMetrics.getParallelism());
			Assert.assertEquals(6, reportMetrics.getInputSequences());
			// 49166 is total set of results if yielder were fully processed,
			// expect somewhere more than 0 but less than that
			// this isn't super indicative of anything really, since closing the
			// yielder would have triggered the baggage
			// to run, which runs this metrics reporter function, while the
			// actual processing could still be occuring on the
			// pool in the background and the yielder still operates as intended
			// if cancellation isn't in fact happening.
			// other tests ensure that this is true though (yielder.next
	...
	private void assertResult(List<Sequence<Pair>> sequences) throws InterruptedException, IOException {
		assertResult(sequences, ParallelMergeCombiningSequence.DEFAULT_TASK_SMALL_BATCH_NUM_ROWS,
				ParallelMergeCombiningSequence.DEFAULT_TASK_INITIAL_YIELD_NUM_ROWS, null);
	}

	private void assertResult(List<Sequence<Pair>> sequences, int batchSize, int yieldAfter)
			throws InterruptedException, IOException {
		assertResult(sequences, batchSize, yieldAfter, null);
	}

	private void assertResult(List<Sequence<Pair>> sequences, int batchSize, int yieldAfter,
			Consumer<ParallelMergeCombiningSequence.MergeCombineMetrics> reporter)
			throws InterruptedException, IOException {
		final CombiningSequence<Pair> combiningSequence = CombiningSequence.create(
				new MergeSequence<>(INT_PAIR_ORDERING, Sequences.simple(sequences)), INT_PAIR_ORDERING,
				INT_PAIR_MERGE_FN);

		final ParallelMergeCombiningSequence<Pair> parallelMergeCombineSequence = new ParallelMergeCombiningSequence<>(
				pool, sequences, INT_PAIR_ORDERING, INT_PAIR_MERGE_FN, true, 5000, 0, TEST_POOL_SIZE, yieldAfter,
				batchSize, ParallelMergeCombiningSequence.DEFAULT_TASK_TARGET_RUN_TIME_MILLIS, reporter);

		Yielder<Pair> combiningYielder = Yielders.each(combiningSequence);
		Yielder<Pair> parallelMergeCombineYielder = Yielders.each(parallelMergeCombineSequence);
	...
	private void assertResultWithEarlyClose(List<Sequence<Pair>> sequences, int batchSize, int yieldAfter,
			int closeYielderAfter, Consumer<ParallelMergeCombiningSequence.MergeCombineMetrics> reporter)
			throws InterruptedException, IOException {
		final CombiningSequence<Pair> combiningSequence = CombiningSequence.create(
				new MergeSequence<>(INT_PAIR_ORDERING, Sequences.simple(sequences)), INT_PAIR_ORDERING,
				INT_PAIR_MERGE_FN);
		...
		try {
			...
			// this should explode so the contradictory next statement should
			// not be reached
	...
	private void assertException(List<Sequence<Pair>> sequences) throws Exception {
		assertException(sequences, ParallelMergeCombiningSequence.DEFAULT_TASK_SMALL_BATCH_NUM_ROWS,
				ParallelMergeCombiningSequence.DEFAULT_TASK_INITIAL_YIELD_NUM_ROWS, 5000L, 0);
	}

	private void assertException(List<Sequence<Pair>> sequences, int batchSize, int yieldAfter, long timeout,
			int readDelayMillis) throws Exception {
		try {
			final ParallelMergeCombiningSequence<Pair> parallelMergeCombineSequence = new ParallelMergeCombiningSequence<>(
					pool, sequences, INT_PAIR_ORDERING, INT_PAIR_MERGE_FN, true, timeout, 0, TEST_POOL_SIZE, yieldAfter,
					batchSize, ParallelMergeCombiningSequence.DEFAULT_TASK_TARGET_RUN_TIME_MILLIS, null);
	...
	/**
	 * Generate an ordered, random valued, non-blocking sequence of
	 * {@link IntPair}, optionally lazy generated with the implication that
	 * every time a sequence is accumulated or yielded it produces
	 * <b>different</b> results, which sort of breaks the {@link Sequence}
	 * contract, and makes this method useless for tests in lazy mode, however
	 * it is useful for benchmarking, where having a sequence without having to
	 * materialize the entire thing up front on heap with a {@link List} backing
	 * is preferable.
	 */
	public static Sequence<Pair> nonBlockingSequence(int size, boolean lazyGenerate) {
		List<Pair> pairs = lazyGenerate ? null : generateOrderedPairs(size);
		return new BaseSequence<>(new BaseSequence.IteratorMaker<Pair, Iterator<Pair>>() {
			@Override
			public Iterator<Pair> make() {
			...
			@Override
			public void cleanup(Iterator<Pair> iterFromMake) {
	...
	/**
	 * Generate an ordered, random valued, blocking sequence of {@link IntPair},
	 * optionally lazy generated. See
	 * {@link ParallelMergeCombiningSequenceTest#nonBlockingSequence(int)} for
	 * the implications of lazy generating a sequence, to summarize each time
	 * the sequence is accumulated or yielded it produces different results.
	 *
	 * This sequence simulates blocking using {@link Thread#sleep(long)}, with
	 * an initial millisecond delay range defined by
	 * {@param startDelayStartMillis} and {@param startDelayEndMillis} that
	 * defines how long to block before the first sequence value will be
	 * produced, and {@param maxIterationDelayMillis} that defines how long to
	 * block every {@param iterationDelayFrequency} rows.
	 */
	public static Sequence<Pair> blockingSequence(int size, int startDelayStartMillis, int startDelayEndMillis,
			int iterationDelayFrequency, int maxIterationDelayMillis, boolean lazyGenerate) {
		final List<Pair> pairs = lazyGenerate ? null : generateOrderedPairs(size);
		final long startDelayMillis = ThreadLocalRandom.current().nextLong(startDelayStartMillis, startDelayEndMillis);
		final long delayUntil = System.nanoTime()
				+ TimeUnit.NANOSECONDS.convert(startDelayMillis, TimeUnit.MILLISECONDS);
		return new BaseSequence<>(new BaseSequence.IteratorMaker<Pair, Iterator<Pair>>() {
			@Override
			public Iterator<Pair> make() {
				return new Iterator<Pair>() {
					...
					public Pair next() {
						try {
							...
							} else if (maxIterationDelayMillis > 0 && rowCounter % iterationDelayFrequency == 0
									&& ThreadLocalRandom.current().nextBoolean()) {
										final int delayMillis = Math
												.max(ThreadLocalRandom.current().nextInt(maxIterationDelayMillis), 1);
	...
	/**
	 * Genenerate non-blocking sequence for tests, non-lazy so the sequence
	 * produces consistent results
	 */
	private static Sequence<Pair> nonBlockingSequence(int size) {
	 ...
	 */
	private static Sequence<Pair> explodingSequence(int explodeAfter) {
		final int explodeAt = explodeAfter + 1;

		// we start at one because we only need to close if the sequence is
		// actually made
		...
		return new ExplodingSequence(new BaseSequence<>(new BaseSequence.IteratorMaker<Pair, Iterator<Pair>>() {
			@Override
			public Iterator<Pair> make() {
				// we got yielder, decrement so we expect it to be incremented
				// again on cleanup
	...
	private static List<Pair> generateOrderedPairs(int length) {
		int rowCounter = 0;
		int mergeKey = 0;
		List<Pair> generatedSequence = new ArrayList<>(length);
	...
	private static Pair makeIntPair(int mergeKey) {
		return Mockito.spy(new Pair(mergeKey, ThreadLocalRandom.current().nextInt(1, 100)));
		// return new IntPair(mergeKey, ThreadLocalRandom.current().nextInt(1,
		// 100));
	}
}
