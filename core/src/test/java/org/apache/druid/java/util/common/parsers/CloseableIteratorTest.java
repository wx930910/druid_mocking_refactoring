...
public class CloseableIteratorTest {
	...
	public void testMap() {
		...
		final Iterator<List<Integer>> expected = IntStream.range(0, 8)
				.mapToObj(i -> IntStream.range(0, i).map(j -> j * 10).boxed().collect(Collectors.toList())).iterator();
	...
	public void testFlatMap() throws IOException {
		...
		final Iterator<Integer> expected = IntStream.range(0, 8).flatMap(i -> IntStream.range(0, i)).iterator();
		while (expected.hasNext() && actual.hasNext()) {
			Assert.assertEquals(expected.next(), actual.next());
		}
		Assert.assertFalse(actual.hasNext());
	...
	public void testFlatMapClosedEarly() throws IOException {
		...
		final Iterator<Integer> expected = IntStream.range(0, numIterations).flatMap(i -> IntStream.range(0, i))
				.iterator();
	...
	private CloseableIterator<Integer> mockCloseableIterator(CloseableIterator<Integer> inner) {
		CloseableIterator<Integer> it = Mockito.mock(CloseableIterator.class);
		try {
		...
		Mockito.when(it.hasNext()).thenAnswer(invo -> {
			return inner.hasNext();
		});
		Mockito.when(it.next()).thenAnswer(invo -> {
			return inner.next();
		});
		return it;
	}
