...
public class WrappingSequenceTest {
	...
	public void testSanity() throws Exception {
		...
		closedCounter.set(0);
		Sequence<Integer> seq = Mockito.mock(Sequence.class, Mockito.CALLS_REAL_METHODS);
		Mockito.when(seq.accumulate(Mockito.any(), Mockito.any())).thenThrow(new UnsupportedOperationException());
		Mockito.when(seq.toYielder(Mockito.any(), Mockito.any())).thenThrow(new UnsupportedOperationException());
		SequenceTestHelper.testClosed(closedCounter, seq.withBaggage(closeable));
	}
	...
	public void testConsistentCloseOrder() {
		...
		Sequence<Integer> sequence = Sequences.simple(Arrays.asList(1, 2, 3))
		...
		// Ensure sequence runs via Yielder, because LimitedSequence extends
