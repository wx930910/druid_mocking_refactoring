...
public class LimitedSequenceTest {
	...
	public void testWithYieldingSequence() {
		...
		// Create a Sequence whose Yielders will yield for each element,
		// regardless of what the accumulator passed
		// to "toYielder" does.
		...
		// Can't use "testAll" because its "testYield" implementation depends on
		// the underlying Sequence _not_ yielding.
	...
	public void testNoSideEffects() {
		...
		Accumulator<Integer, Integer> accu = Mockito.mock(Accumulator.class);
		Mockito.when(accu.accumulate(Mockito.anyInt(), Mockito.anyInt())).thenAnswer(invo -> {
			return (int) invo.getArgument(0) + (int) invo.getArgument(1);
		});
		Assert.assertEquals(10, seq.accumulate(0, accu).intValue());
