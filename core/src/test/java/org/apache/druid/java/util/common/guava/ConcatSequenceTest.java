...
public class ConcatSequenceTest {
	...
	public void testAccumulationMultipleAndEmpty() throws Exception {
		testAll(Arrays.asList(Arrays.asList(1, 2, 3, 4, 5), Collections.emptyList(), Arrays.asList(6, 7, 8),
	...
	public void testAccumulationMultipleAndEmpty1() throws Exception {
		testAll(Arrays.asList(Arrays.asList(1, 2, 3, 4, 5), Collections.emptyList(), Arrays.asList(6, 7, 8),
				Arrays.asList(9, 10, 11, 12), Collections.emptyList()));
	}

	@Test
	public void testAccumulationMultipleAndEmpty2() throws Exception {
		testAll(Arrays.asList(Collections.emptyList(), Arrays.asList(1, 2, 3, 4, 5), Collections.emptyList(),
				Arrays.asList(6, 7, 8), Arrays.asList(9, 10, 11, 12)));
	}
		...
		final Sequence<Integer> seq = Sequences.concat(
				new BaseSequence<>(new BaseSequence.IteratorMaker<Sequence<Integer>, Iterator<Sequence<Integer>>>() {
					...
					public Iterator<Sequence<Integer>> make() {
						return Arrays.asList(Sequences.simple(Arrays.asList(1, 2, 3, 4)),
								Sequences.simple(Arrays.asList(5, 6, 7, 8))).iterator();
					}
	...
	public void testClosingOfSequenceSequenceWhenExceptionThrown() {
		...
		final Sequence<Integer> seq = Sequences.concat(
				new BaseSequence<>(new BaseSequence.IteratorMaker<Sequence<Integer>, Iterator<Sequence<Integer>>>() {
					@Override
					public Iterator<Sequence<Integer>> make() {
						return Arrays.asList(Sequences.simple(Arrays.asList(1, 2, 3, 4)), new UnsupportedSequence())
								.iterator();
					}
	...
	public void testEnsureNextSequenceIsCalledLazilyInToYielder() throws Exception {
		...
		Sequence<Integer> seq = Sequences.concat(
				Sequences.map(Sequences.simple(ImmutableList.of(ImmutableList.of(1, 2, 3), ImmutableList.of(4, 5, 6))),
						new Function<ImmutableList<Integer>, Sequence<Integer>>() {
							...
							public Sequence<Integer> apply(final ImmutableList<Integer> input) {
								if (lastSeqFullyRead.getAndSet(false)) {
									return Sequences.simple(new Iterable<Integer>() {
		...
		Yielder<Integer> yielder = seq.toYielder(null, new YieldingAccumulator<Integer, Integer>() {
			@Override
			public Integer accumulate(Integer accumulated, Integer in) {
				yield();
				return in;
	...
	public void testAll(Iterable<List<Integer>> vals) throws IOException {
		final Iterable<TestSequence<Integer>> theSequences = Iterables.transform(vals,
		...
		SequenceTestHelper.testYield("", new ConcatSequence<Integer>((Sequence) Sequences.simple(yieldSeqs)),
				Lists.newArrayList(Iterables.concat(vals)));
