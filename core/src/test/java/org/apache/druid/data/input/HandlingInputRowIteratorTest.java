...
public class HandlingInputRowIteratorTest {
	...
	public static class PresentRowTest {
		...
		private boolean[] successful = new boolean[1];
		private boolean[] successfulInvoked = new boolean[1];
		private boolean[] unsuccessful = new boolean[1];
		private boolean[] unsuccessfulInvoked = new boolean[1];
		...
		public void setup() {
			...
			Mockito.when(successfulHandler.handle(Mockito.any())).thenAnswer(invo -> {
				successfulInvoked[0] = true;
				return successful[0];
			});
			Mockito.when(unsuccessfulHandler.handle(Mockito.any())).thenAnswer(invo -> {
				unsuccessfulInvoked[0] = true;
		...
		public void hasNext() {
			...
			Assert.assertFalse(unsuccessfulInvoked[0]);
		}
		...
		public void yieldsNextIfUnhandled() {
			...
			Assert.assertTrue(unsuccessfulInvoked[0]);
		}
		...
		public void yieldsNullIfHandledByFirst() {
			...
			Assert.assertTrue(successfulInvoked[0]);
			Assert.assertFalse(unsuccessfulInvoked[0]);
		}
