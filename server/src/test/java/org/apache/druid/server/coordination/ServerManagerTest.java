...
public class ServerManagerTest {
	...
	public static class MyQueryRunnerFactory implements QueryRunnerFactory<Result<SearchResultValue>, SearchQuery> {
		...
		public QueryRunner<Result<SearchResultValue>> createRunner(Segment adapter) {
			...
			return mockQueryRunner(new NoopQueryRunner<>(), waitLatch, waitYieldLatch, notifyLatch);
		}
	...
	private static QueryToolChest<Result<SearchResultValue>, SearchQuery> mockQueryToolChest() {
		QueryToolChest<Result<SearchResultValue>, SearchQuery> res = Mockito.mock(QueryToolChest.class,
				Mockito.CALLS_REAL_METHODS);
		Mockito.when(res.mergeResults(Mockito.any())).thenAnswer(invo -> {
			return invo.getArgument(0);
		});
		Mockito.when(res.makeMetrics(Mockito.any())).thenAnswer(invo -> {
			return new DefaultQueryMetrics<>();
		});
		Mockito.when(res.makePreComputeManipulatorFn(Mockito.any(), Mockito.any())).thenAnswer(invo -> {
			return Functions.identity();
		});
		Mockito.when(res.getResultTypeReference()).thenAnswer(invo -> {
	...
	private static QueryRunner<Result<SearchResultValue>> mockQueryRunner(QueryRunner<Result<SearchResultValue>> runner,
			CountDownLatch waitLatch, CountDownLatch waitYieldLatch, CountDownLatch notifyLatch) {
		QueryRunner<Result<SearchResultValue>> res = Mockito.mock(QueryRunner.class);
		Mockito.when(res.run(Mockito.any(), Mockito.any())).thenAnswer(invo -> {
			QueryPlus<Result<SearchResultValue>> queryPlus = invo.getArgument(0);
			ResponseContext responseContext = invo.getArgument(1);
			return new BlockingSequence<>(runner.run(queryPlus, responseContext), waitLatch, waitYieldLatch,
					notifyLatch);
		});
		return res;
	}
	...
	private static class BlockingQueryRunner<T> implements QueryRunner<T> {
		...
		public BlockingQueryRunner(QueryRunner<T> runner, CountDownLatch waitLatch, CountDownLatch waitYieldLatch,
				CountDownLatch notifyLatch) {
			this.runner = runner;
			this.waitLatch = waitLatch;
			this.waitYieldLatch = waitYieldLatch;
