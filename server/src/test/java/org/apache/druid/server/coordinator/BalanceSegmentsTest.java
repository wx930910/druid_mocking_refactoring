...
public class BalanceSegmentsTest {
	...
	public void testMoveToEmptyServerBalancer() {
		...
		BalancerStrategy predefinedPickOrderStrategy = mockBalancerStrategy(balancerStrategy, ImmutableList.of(
				new BalancerSegmentHolder(druidServer1, segment1), new BalancerSegmentHolder(druidServer1, segment2),
				new BalancerSegmentHolder(druidServer1, segment3), new BalancerSegmentHolder(druidServer1, segment4)));
	...
	/**
	 * Server 1 has 2 segments. Server 2 (decommissioning) has 2 segments.
	 * Server 3 is empty. Decommissioning percent is 60. Max segments to move is
	 * 3. 2 (of 2) segments should be moved from Server 2 and 1 (of 2) from
	 * Server 1.
	 */
	@Test
		...
		DruidCoordinatorRuntimeParams params = defaultRuntimeParamsBuilder(
				...
				ImmutableList.of(false, true, false))
						.withDynamicConfigs(CoordinatorDynamicConfig.builder().withMaxSegmentsToMove(3)
								.withDecommissioningMaxPercentOfMaxSegmentsToMove(60).build() // ceil(3
																								// *
																								// 0.6)
																								// =
																								// 2
	...
	public void testMoveSameSegmentTwice() {
		...
		BalancerStrategy predefinedPickOrderStrategy = mockBalancerStrategy(balancerStrategy,
	...
	private BalancerStrategy mockBalancerStrategy(BalancerStrategy delegate, List<BalancerSegmentHolder> pickOrder) {
		BalancerStrategy res = Mockito.mock(BalancerStrategy.class);
		AtomicInteger pickCounter = new AtomicInteger(0);
		Mockito.when(res.findNewSegmentHomeBalancer(Mockito.any(), Mockito.anyList())).thenAnswer(invo -> {
			DataSegment proposalSegment = invo.getArgument(0);
			List<ServerHolder> serverHolders = invo.getArgument(1);
			return delegate.findNewSegmentHomeBalancer(proposalSegment, serverHolders);
		});
		Mockito.when(res.findNewSegmentHomeReplicator(Mockito.any(), Mockito.anyList())).thenAnswer(invo -> {
			DataSegment proposalSegment = invo.getArgument(0);
			List<ServerHolder> serverHolders = invo.getArgument(1);
			return delegate.findNewSegmentHomeReplicator(proposalSegment, serverHolders);
		});
		Mockito.when(res.pickSegmentToMove(Mockito.anyList(), Mockito.anySet())).thenAnswer(invo -> {
			Set<String> broadcastDatasources = invo.getArgument(1);
			List<ServerHolder> serverHolders = invo.getArgument(0);
			return pickOrder.get(pickCounter.getAndIncrement() % pickOrder.size());
		});
		Mockito.doAnswer(invo -> {
			String tier = invo.getArgument(0);
			CoordinatorStats stats = invo.getArgument(1);
			List<ServerHolder> serverHolderList = invo.getArgument(2);
			delegate.emitStats(tier, stats, serverHolderList);
			return null;
		}).when(res).emitStats(Mockito.anyString(), Mockito.any(), Mockito.anyList());
		return res;
	}
	...
	private DruidCoordinatorRuntimeParams setupParamsForDecommissioningMaxPercentOfMaxSegmentsToMove(int percent) {
		...
		// either decommissioning servers list or acitve ones (ie servers list
		// is [2] or [1, 3])
		BalancerStrategy strategy = EasyMock.createMock(BalancerStrategy.class);
