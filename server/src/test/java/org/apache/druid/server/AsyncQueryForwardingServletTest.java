...
public class AsyncQueryForwardingServletTest extends BaseJettyTest {
	...
	public void testQueryProxy() throws Exception {
		...
		EasyMock.expect(hostFinder.pickServer(query)).andReturn(mockServer("http", "1.2.3.4", 9999)).once();
		...
		final AsyncQueryForwardingServlet servlet = new AsyncQueryForwardingServlet(
				...
				null, null, new NoopServiceEmitter(), new NoopRequestLogger(), new DefaultGenericQueryMetricsFactory(),
		...
		public void initialize(Server server, Injector injector) {
			...
			final QueryHostFinder hostFinder = new QueryHostFinder(null,
				...
				public Collection<org.apache.druid.client.selector.Server> getAllServers() {
					return ImmutableList.of(new TestServer("http", "localhost", node.getPlaintextPort()),
							new TestServer("http", "localhost", port1), new TestServer("http", "localhost", port2));
				}
			};
			...
			ServletHolder holder = new ServletHolder(
					new AsyncQueryForwardingServlet(new MapQueryToolChestWarehouse(ImmutableMap.of()), jsonMapper,
							...
							new NoopServiceEmitter(), new NoopRequestLogger(), new DefaultGenericQueryMetricsFactory(),
							new AuthenticatorMapper(ImmutableMap.of())) {
						@Override
	...
	private static org.apache.druid.client.selector.Server mockServer(String scheme, String address, int port) {
		org.apache.druid.client.selector.Server res = Mockito.mock(org.apache.druid.client.selector.Server.class);
		Mockito.when(res.getScheme()).thenAnswer(invo -> {
			return scheme;
		});
		Mockito.when(res.getHost()).thenAnswer(invo -> {
			return address + ":" + port;
		});
		Mockito.when(res.getAddress()).thenAnswer(invo -> {
			return address;
		});
		Mockito.when(res.getPort()).thenAnswer(invo -> {
			return port;
		});
		return res;
	}
