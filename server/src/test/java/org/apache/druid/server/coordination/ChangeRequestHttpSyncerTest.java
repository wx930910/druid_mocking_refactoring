...
public class ChangeRequestHttpSyncerTest {
	...
	public void testSimple() throws Exception {
		...
		TestHttpClient httpClient = new TestHttpClient(ImmutableList.of(
				Futures.immediateFuture(new ByteArrayInputStream(jsonMapper.writerWithType(typeRef)
				...
				Futures.immediateFuture(new ByteArrayInputStream(jsonMapper.writerWithType(typeRef)
						.writeValueAsBytes(new ChangeRequestsSnapshot(false, null, ChangeRequestHistory.Counter.ZERO,
								ImmutableList.of("s2"))))),
				Futures.immediateFuture(new ByteArrayInputStream(jsonMapper.writerWithType(typeRef)
						.writeValueAsBytes(new ChangeRequestsSnapshot(true, "reset the counter",
								ChangeRequestHistory.Counter.ZERO, ImmutableList.of())))),
				Futures.immediateFuture(new ByteArrayInputStream(jsonMapper.writerWithType(typeRef)
						.writeValueAsBytes(new ChangeRequestsSnapshot(false, null, ChangeRequestHistory.Counter.ZERO,
								ImmutableList.of("s3"))))),
				Futures.immediateFuture(new ByteArrayInputStream(
						jsonMapper.writerWithType(typeRef).writeValueAsBytes(new ChangeRequestsSnapshot(false, null,
								ChangeRequestHistory.Counter.ZERO, ImmutableList.of("s4")))))));
		...
		ChangeRequestHttpSyncer<String> syncer = new ChangeRequestHttpSyncer<>(jsonMapper, httpClient,
				Execs.scheduledSingleThreaded("ChangeRequestHttpSyncerTest"), new URL("http://localhost:8080/"), "/xx",
				typeRef, 50000, 10000, listener);
