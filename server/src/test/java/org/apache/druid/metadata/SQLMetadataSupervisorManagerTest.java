...
public class SQLMetadataSupervisorManagerTest {
	...
	public void testInsertAndGet() {
		...
		// add 2 supervisors, one revision each, and make sure the state is as
		// expected
	...
	public void testSkipDeserializingBadSpecs() {
		...
		Mockito.when(spec.getId()).thenReturn(supervisor2);
		Mockito.when(spec.createSupervisor()).thenThrow(new UnsupportedOperationException());
		Mockito.when(spec.getDataSources()).thenAnswer(invo -> {
			return Collections.singletonList(supervisor2);
		});

		supervisorManager.insert(supervisor1, new TestSupervisorSpec(supervisor1, data1rev1));
		supervisorManager.insert(supervisor2, spec);
		supervisorManager.insert(supervisor1, new TestSupervisorSpec(supervisor1, data1rev2));
