...
public class SQLMetadataConnectorTest {
	...
	static class TestSQLMetadataConnector extends SQLMetadataConnector {
		public TestSQLMetadataConnector(Supplier<MetadataStorageConnectorConfig> config,
	...
	public void testBasicDataSourceCreation() throws Exception {
		...
		SQLMetadataConnector testSQLMetadataConnector = Mockito.mock(SQLMetadataConnector.class,
				Mockito.withSettings().defaultAnswer(Mockito.CALLS_REAL_METHODS)
						.useConstructor(Suppliers.ofInstance(config), Suppliers.ofInstance(tablesConfig)));
		Mockito.when(testSQLMetadataConnector.getSerialType()).thenReturn(null);
		Mockito.when(testSQLMetadataConnector.getStreamingFetchSize()).thenReturn(0);
		Mockito.when(testSQLMetadataConnector.getQuoteString()).thenReturn(null);
		Mockito.when(testSQLMetadataConnector.tableExists(Mockito.any(), Mockito.anyString())).thenReturn(false);
		Mockito.when(testSQLMetadataConnector.getDBI()).thenReturn(null);
		Mockito.when(testSQLMetadataConnector.getDatasource()).thenAnswer(invo -> invo.callRealMethod());
		// TestSQLMetadataConnector testSQLMetadataConnector = new
		// TestSQLMetadataConnector(Suppliers.ofInstance(config),
